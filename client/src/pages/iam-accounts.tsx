  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      ...
      },
      {
        ...
        knoxId: "KNOX003",
        permission: "S3:ListBucket,S3:GetObject",
    ...
    const csvContent = [
      ...
      ...templateData.map(row => 
        `"${row.requestor.replace(/"/g, '""')}","${row.knoxId}","${row.permission.replace(/"/g, '""')}",${row.durationStartDate},${row.durationEndDate},"${row.cloudPlatform}","${row.projectAccounts.replace(/"/g, '""')}","${row.approvalId}","${row.remarks.replace(/"/g, '""')}",${row.status}`
      )
    ].join('\n');
        ...
        headers.forEach((header, index) => {
          ...
          switch (header) {
            ...
            case 'notes':
              ...
              break;
            case 'status':
              account.status = value === '-' ? 'active' : value;
              break;
          }
        });
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4">
            ...
            <div className="text-xs text-gray-500 space-y-1">
              <p><strong>Available columns:</strong> requestor, knoxId, permission, cloudPlatform, durationStartDate, durationEndDate, projectAccounts, approvalId, remarks, status</p>
